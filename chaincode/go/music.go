...
func generateKey(APIstub shim.ChaincodeStubInterface, key string) []byte {
	...
	var templdx string
	templdx = strconv.Itoa(musickey.Idx)
	...
	fmt.Println("Last MusicKey is " + musickey.Key + " : " + templdx)
...
func (s *SmartContract) setMusic(APIstub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	keyidx := strconv.Itoa(musickey.Idx) // int -> string
	...
	if err != nil {
		return shim.Error(fmt.Sprintf("Failded to record music catch: %s", musickey))
	}
...
func (s *SmartContract) getAllMusic(APIstub shim.ChaincodeStubInterface) pb.Response {
	...
	resultslter, err := APIstub.GetStateByRange(startKey, endKey)
	if err != nil {
		return shim.Error(err.Error())
	}
	defer resultslter.Close()

	var buffer bytes.Buffer
	buffer.WriteString("[")
	bArrayMemberAlreadyWritten := false
	for resultslter.HasNext() {
		queryResponse, err := resultslter.Next()
		if err != nil {
			return shim.Error(err.Error())
		}
